;(function($){
	...
	$.fn.lxCarousel = function(options){
		...
		var defaults = {
			...
			duration:2000,//ok
			index:0,//ok
			type:'vertical',//vertical,horizontal,fade,show,手动滚动（滚动）
			seamless:false
		}
		...
		return this.each(function(idx,ele){
			...
			//淡入淡出,图片重叠
			if(opt.type === 'fadeShow'){
				$ul.children().addClass('overlap');
			}
			...
			function showPic(){
				...
				if(opt.type === 'horizontal'){
					...
					$ul.stop().animate(obj);
				}else if(opt.type === 'fadeShow'){	
					/*obj = {opacity:0};
					bj={opacity:1};
					$ul.children().eq(index).stop().animate(obj);
					console.log($ul.children().eq(index))*/
					var ind=index-1;
